...
- **Phase 4 ✅ done** — Provider-agnostic `LLMProvider` interface with Gemini (primary, free tier) + Anthropic (secondary, `claude-sonnet-4-6`) adapters selected via `LLM_PROVIDER` env var. `withBackoff` transport retry, `toGeminiSchema()` normalizer. Six metadata-only tool definitions in `packages/tools` bound to in-process services via `createAgentToolRegistry`. `ToolBroker` validates with Ajv and surfaces failures as `is_error` tool results. `AgentService` runs the multi-hop loop with a 3-strike self-correction cap. `POST /agent/chat` streams `AgentEvent`s as SSE; `useAgentChat` consumes it; `AgentChatPanel` renders the transcript with live tool-call chips. Missing API key → agent layer disabled at startup, route returns 503.
- **Currently: Phase 5** — MCP server. See `docs/IMPLEMENTATION_PLAN.md` for per-phase deliverable checkboxes; that file is the source of truth for what's done vs. pending within a phase. Update this block when a phase flips.
