...
- **Currently: Phase 4** — AI Agent Layer (provider-agnostic `LLMProvider`, Gemini primary + Anthropic secondary, `ToolBroker`, agent orchestrator, chat panel). See `docs/IMPLEMENTATION_PLAN.md` for per-phase deliverable checkboxes; that file is the source of truth for what's done vs. pending within a phase. Update this block when a phase flips.
