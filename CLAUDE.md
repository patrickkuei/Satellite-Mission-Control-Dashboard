...
- **Phase 5 ✅ done** — MCP server. `packages/mcp-server` exposes all 6 tools via `@modelcontextprotocol/sdk` stdio transport, with HTTP execute bindings to `apps/api`. Three new REST endpoints added to support it: `GET /telemetry/snapshot`, `GET /anomalies`, `GET /satellites/above`. Claude Desktop config documented in `packages/mcp-server/README.md`. Pending: live Claude Desktop verification + demo video (Phase 5 post-build steps).
- **Currently: Phase 6** — Polish + Ship. See `docs/IMPLEMENTATION_PLAN.md` for per-phase deliverable checkboxes; that file is the source of truth for what's done vs. pending within a phase. Update this block when a phase flips.
